...
Voidable ships 74 components organized by category. All components render in the Light DOM and are styled entirely through `@voidable/theme` CSS tokens.
...
| Multiselect | `<void-multiselect>` | Multi-value picker for tag selection and filter fields; shows chips for selected items |
| Combobox | `<void-combobox>` | Filterable dropdown for long option lists like country selectors and user pickers |
...
| SegmentedControl | `<void-segmented-control>` | Mutually exclusive button group for view mode toggles and compact tab-like navigation |
...
| DropdownMenu | `<void-dropdown-menu>` | Trigger-anchored action menu for table row actions and toolbar overflow menus |
| Menu / MenuItem | `<void-menu>` | Always-visible menu panel for command palettes, side panels, and searchable action lists |
