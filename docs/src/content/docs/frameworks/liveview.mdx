...
The preferred approach for preserving client-managed state is `VoidDOM`, which is included in `VoidHooks`. It automatically prevents LiveView from clobbering internal component state (open dialogs, checked states, focus) without needing `phx-update="ignore"` on every element.
