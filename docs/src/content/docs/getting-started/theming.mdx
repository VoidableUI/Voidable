...
Voidable separates structure from style. Components ship with **zero visual CSS** — all appearance is driven by `--void-*` CSS custom properties defined in `@voidable/theme`.

## Token architecture

The theme is organized into three tiers:

1. **Primitives** (`primitives.css`) — Raw scales for colors (`--void-stone-500`), spacing (`--void-space-4`), typography, border radius, shadows, motion, and z-index. No semantic meaning.
